...
while a < 4: # Цикл для расчета среднего возраста юзеров
    ...
    average_age = average_age + int(user_list1['age'])
    a = a + 1
...
Y = input ('Введите номер пользователя, которого нужно переместить в конец: ')
print(user_list[int(Y)]) # Вывод юзера для переноса в конец списка
print(' ')

# Вывод списка юзеров до изменения
i = 0
while i < 4:
    user_listY = user_list[i]
    print (user_listY)
    i = i + 1

# Перенос юзера в конец списка
print(' ')
user_listY = user_list.pop(int(Y))
user_list.append(user_listY)
